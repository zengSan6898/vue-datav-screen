...
export default function() {
  ...
  const update = () => {
    ...
    // 年龄分布数据
    const _ageData = [];
    ageMockData.forEach((item, index) => {
    ...
    // 登录设备数据
    deviceData.value = {
      totalDevices: deviceMockData.totalDevices,
      devices: deviceMockData.devices
    };

    // 性别分布
    genderData.value = genderMockData;

    // 骑手数据
    riderData.value = riderMockData;

    // 热门分类数据
    hotCategoryData.value = hotCategoryMockData;

    // 核心指标数据
    headerData.value = headerMockData;

    // 实时订单数据
    console.log("实时订单数据-----", realTimeOrderData.value.date);
    realTimeOrderData.value = {
      date: [...realTimeOrderMockData.date, getNowTime()],
      data: [...realTimeOrderMockData.data, realTimeOrderMockData]
    };

    // 销售列表数据
    salesListData.value = salesListMockData;

    // 区域销售排行数据
    salesRankData.value = salesRankMockData;

    scheduleViewData.value = scheduleViewMockData;
    // });
  };
